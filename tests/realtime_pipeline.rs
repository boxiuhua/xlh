...
fn quiet_stock_with_flat_volume_produces_no_signal() {
    ...
    let hist =
        store::same_slot_deltas(&c, "600000", (10, 10), d(2026, 7, 16), d(2026, 7, 6)).unwrap();
    let (jump, surge, _, _, _) = movers::compute(&recent, &hist, None).unwrap();

    assert!(
        !movers::is_mover(jump, surge, cfg().price_jump_pct, cfg().volume_surge_x),
...
fn price_spike_with_volume_surge_is_detected_and_stored() {
    ...
    let hist =
        store::same_slot_deltas(&c, "600001", (10, 10), d(2026, 7, 16), d(2026, 7, 6)).unwrap();
    ...
    assert_eq!(
        base,
        movers::Baseline::History,
        "有历史样本就该用历史，不该 fallback"
    );
    assert!(movers::is_mover(
        jump,
        surge,
        cfg().price_jump_pct,
        cfg().volume_surge_x
    ...
    assert!(
        (rows[0].trigger_price - 10.5).abs() < 1e-9,
        "触发价须落库，回填结局要用它"
    );
}
...
fn u_shaped_volume_does_not_cause_false_positive_at_quiet_slot() {
    ...
    let hist =
        store::same_slot_deltas(&c, "600002", (14, 10), d(2026, 7, 16), d(2026, 7, 6)).unwrap();
    ...
    assert!(
        wrong_surge < 0.05,
        "用累计量当基准会把放大倍数稀释到 {wrong_surge:.3}，永不触发"
    );
}
...
fn cold_start_falls_back_and_marks_it() {
    ...
    let hist =
        store::same_slot_deltas(&c, "600003", (10, 10), d(2026, 7, 16), d(2026, 7, 6)).unwrap();
    assert!(hist.is_empty(), "冷启动无历史样本");

    let (_, _, base, _, _) = movers::compute(&recent, &hist, Some(500.0)).unwrap();
    assert_eq!(
        base,
        movers::Baseline::Fallback,
...
fn ten_day_prune_keeps_signals_and_drops_old_ticks() {
    ...
    let old = movers::Mover {
        ...
        main_net: None,
        main_net_pct: None,
        divergence: movers::Divergence::Unknown,
        horizon: movers::Horizon::Short,
    ...
    assert_eq!(
        remaining, 2,
        "7/1、7/2 的 ticks 超出 10 天须删，7/15 的保留"
    );
    assert_eq!(
        sigs.len(),
        1,
        "7/1 的信号必须永久保留 —— 它是验证阈值的唯一依据"
    );
}
...
fn full_day_flow_from_detection_to_summary() {
    ...
    assert_eq!(
        ...
        movers::Divergence::RetailChasing,
        "涨 +5% 但主力净流出 8% → 散户抬轿"
    );
    ...
    let md = job::render_summary(
        &store::signals_on(&c, d(2026, 7, 16)).unwrap(),
        d(2026, 7, 16),
    );
...
fn holiday_marking_blocks_rest_of_day_but_premarket_does_not() {
    ...
    assert_eq!(
        job::should_run(&c, dt(16, 14, 0)).unwrap(),
        Some(job::Skip::KnownHoliday)
    );
