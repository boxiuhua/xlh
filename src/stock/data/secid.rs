...
impl Secid {
    ...
    pub fn cache_key(&self) -> String {
        format!("{}_{}", self.market, self.code)
    }
    ...
    if s.chars().all(|c| c.is_ascii_digit()) {
        return match s.len() {
            6 => {
                ...
                Ok(Resolved::Ready(Secid {
                    market,
                    code: s.to_string(),
                }))
            }
            n if n < 6 => Ok(Resolved::Ready(Secid {
                market: 116,
                code: pad5(s),
            })),
...
mod tests {
    ...
    fn ready(input: &str) -> Secid {
        match resolve_offline(input).unwrap() {
            Resolved::Ready(s) => s,
            _ => panic!("应离线可解析: {input}"),
        }
    }
    ...
    fn us_ticker_needs_search() {
        assert_eq!(
            resolve_offline("AAPL").unwrap(),
            Resolved::NeedSearch("AAPL".into())
        );
        assert_eq!(
            resolve_offline("us.tsla").unwrap(),
            Resolved::NeedSearch("TSLA".into())
        );
    }
    ...
    fn us_tickers_colliding_with_prefixes_still_search() {
        ...
        assert_eq!(
            resolve_offline("SHOP").unwrap(),
            Resolved::NeedSearch("SHOP".into())
        );
        assert_eq!(
            resolve_offline("USB").unwrap(),
            Resolved::NeedSearch("USB".into())
        );
    }
