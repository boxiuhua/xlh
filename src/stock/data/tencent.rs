...
pub fn parse_array(body: &str, key: &str) -> Result<Vec<Row>> {
    ...
    let data = v
        ...
        .ok_or_else(|| anyhow!("腾讯响应缺 data"))?;
    // data 下唯一键即符号（如 "sh600519"）
    let node = data
        .values()
        .next()
        .and_then(|n| n.as_object())
        .ok_or_else(|| anyhow!("腾讯响应空"))?;
    let Some(arr) = node.get(key).and_then(|a| a.as_array()) else {
        return Ok(Vec::new());
    };
    let mut rows = Vec::with_capacity(arr.len());
    for item in arr {
        // 每行：[date, open, close, high, low, volume, (可选分红对象)]
        let Some(cols) = item.as_array() else {
            continue;
        };
        if cols.len() < 6 {
            continue;
        ...
        let (Some(date), Some(open), Some(close), Some(high), Some(low), Some(volume)) =
            (date, num(1), num(2), num(3), num(4), num(5))
        else {
            continue;
        };
        rows.push(Row {
...
mod tests {
    ...
    fn merge_clip_falls_back_to_raw_when_no_hfq() {
        ...
        assert!(
            (bars[0].adj_close - bars[0].close).abs() < 1e-9,
            "adj_close 回退 raw close"
        );
    }
}
