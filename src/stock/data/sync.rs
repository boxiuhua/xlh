...
fn sync_one(display: &str, secid: &Secid, cache_dir: &Path) -> SyncOutcome {
    ...
    let cached = if path.exists() {
        super::cache::read_csv(&path).unwrap_or_default()
    } else {
        Vec::new()
    };
    let fresh = match super::kline::fetch(secid) {
        ...
        Err(e) => {
            return SyncOutcome {
                ...
                added: 0,
                total: cached.len(),
                latest: cached.last().map(|b| b.date.to_string()),
...
pub fn sync_all(cache_dir: &Path) -> Vec<SyncOutcome> {
    ...
    keys.sort();
    keys.iter()
        .filter_map(|k| Secid::from_cache_key(k).map(|s| sync_one(k, &s, cache_dir)))
        .collect()
}
