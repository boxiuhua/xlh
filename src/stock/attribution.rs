...
pub fn attribute(
    ...
    let b0 = bars
        .iter()
        .find(|b| b.date >= start)
        .ok_or(AttributionError::MissingPrice)?;
    let b1 = bars
        .iter()
        .rev()
        .find(|b| b.date <= end)
        .ok_or(AttributionError::MissingPrice)?;
    if b1.date <= b0.date {
        return Err(AttributionError::MissingPrice);
    }

    let days = (b1.date - b0.date).num_days() as f64;
    let years = days / 365.25;
    if years < 1.0 {
        return Err(AttributionError::TooShort);
    }
    ...
    if pe0 <= 0.0 || pe1 <= 0.0 {
        return Err(AttributionError::NonPositiveEarnings);
    }

    if b0.close <= 0.0 || b0.adj_close <= 0.0 {
        return Err(AttributionError::MissingPrice);
    }
    ...
    match start_pe_percentile {
        ...
        None => caveats
            .push("该股历史估值样本不足，起点分位无法计算，归因的起点敏感性无从评估。".to_string()),
        _ => {}
    }
    if total_multiple.is_none() {
        caveats.push(
            "该区间无后复权数据，**无法计算分红再投资的贡献** —— 下列倍数与占比均为\
             裸价格口径，会系统性低估真实总回报。（参考：茅台 2001–2017 裸价格仅涨 10.3 倍，\
             含分红再投资的总回报是 70 倍。）当前数据源的后复权覆盖范围有限，\
             若需长周期总回报口径，须换用覆盖更长的后复权数据源。"
                .to_string(),
        );
    }
...
impl Attribution {
    ...
    pub fn explain(&self) -> String {
        ...
        let val_word = if self.valuation_multiple >= 1.0 {
            "扩张"
        } else {
            "收缩"
        };
        ...
        let breakdown = match (self.earnings_share, self.valuation_share) {
            ...
            _ => format!(
                ...
                self.valuation_multiple,
                if self.earnings_multiple > 1.0 {
                    "吃光还倒贴"
                } else {
                    "进一步放大了亏损"
                }
...
mod tests {
    ...
    fn live_attribute_moutai() {
        ...
        assert!(
            (a.earnings_multiple * a.valuation_multiple - a.price_multiple).abs() < 1e-6,
            "盈利 × 估值 必须等于裸价格"
        );
    ...
    fn pe_history(base: f64, n: i64) -> Vec<ValPoint> {
        (0..n)
            .map(|i| {
                vp(
                    d(2016, 1, 1) + chrono::Duration::days(i),
                    base + (i % 20) as f64,
                )
            })
            .collect()
    ...
    fn identity_holds_earnings_times_valuation_equals_price() {
        ...
        let bars = vec![
            bar(d(2010, 1, 4), 10.0, 12.0),
            bar(d(2020, 1, 6), 100.0, 130.0),
        ];
    ...
    fn log_shares_sum_to_one() {
        ...
        assert_eq!(a.share_basis, ShareBasis::TotalReturn);
        let sum =
            a.earnings_share.unwrap() + a.valuation_share.unwrap() + a.dividend_share.unwrap();
        assert!(
            (sum - 1.0).abs() < 1e-9,
            "三项对数占比须相加为 1，实得 {sum}"
        );
    ...
    fn loss_window_must_not_report_sign_flipped_shares() {
        ...
        let bars = vec![
            bar(d(2023, 11, 17), 1000.0, 1000.0),
            bar(d(2026, 7, 10), 700.0, 812.0),
        ];
    ...
    fn separates_total_return_from_naked_price() {
        ...
        let bars = vec![
            bar(d(2001, 8, 27), 35.55, 35.55),
            bar(d(2017, 3, 8), 366.2, 2572.22),
        ];
        let mut vals = pe_history(30.0, 80);
        ...
        assert!(
            (a.total_multiple.unwrap() - 72.35).abs() < 0.1,
            "总回报(后复权) ≈ 72 倍"
        );
        assert!(
            (a.price_multiple - 10.3).abs() < 0.1,
            "裸价格只有 ≈ 10.3 倍"
        );
        assert!(a.dividend_multiple.unwrap() > 6.0, "差额来自分红再投资");
        assert!(
            a.annualized > 25.0 && a.annualized < 35.0,
            "年化应在 30% 附近，实得 {}",
            a.annualized
        );
    }
    ...
    fn missing_hfq_must_not_masquerade_as_zero_dividends() {
        ...
        let bars = vec![
            bar(d(2010, 1, 4), 10.0, 10.0),
            bar(d(2020, 1, 6), 100.0, 100.0),
        ];
        let mut vals = pe_history(20.0, 80);
        vals.push(vp(d(2010, 1, 4), 20.0));
        vals.push(vp(d(2020, 1, 6), 25.0));
        ...
        assert_eq!(
            a.dividend_multiple, None,
            "算不出来必须是 None，绝不能是 1.0"
        );
        assert_eq!(a.total_multiple, None, "没有分红数据就没有总回报可言");
        assert_eq!(a.dividend_share, None);
        assert_eq!(
            a.share_basis,
            ShareBasis::PriceOnly,
            "占比须标明是裸价格口径"
        );
    ...
    fn warns_when_start_pe_is_at_historic_low() {
        ...
        assert!(
            p < LOW_START_PERCENTILE,
            "起点 PE 9 应落在极低分位，实得 {p}"
        );
    ...
    fn refuses_sub_year_windows() {
        let bars = vec![
            bar(d(2020, 1, 4), 10.0, 10.0),
            bar(d(2020, 6, 6), 20.0, 20.0),
        ];
        let mut vals = pe_history(20.0, 80);
    ...
    fn explain_reports_both_calibers_and_the_caveat() {
        ...
        let text = attribute(&bars, &vals, d(2014, 1, 1), d(2020, 1, 10))
            .unwrap()
            .explain();
