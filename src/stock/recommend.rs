...
fn split_history(bars: &[StockBar], split_ratio: f64) -> Option<(&[StockBar], &[StockBar])> {
    ...
    if train.len() >= MIN_TRAIN && test.len() >= MIN_TEST {
        Some((train, test))
    } else {
        None
    }
...
pub fn evaluate_stock(
    ...
    bars: &[StockBar],
    p: &RecommendParams,
) -> anyhow::Result<StockRecommendation> {
    ...
    for (kind, name_cn) in CANDIDATES {
        ...
        evals.push(StockStrategyEval {
            ...
            is_return: is_s.total_return,
            is_sharpe: is_s.sharpe,
            is_mdd: is_s.max_drawdown,
            oos_return: oos_s.total_return,
            oos_sharpe: oos_s.sharpe,
            oos_mdd: oos_s.max_drawdown,
            score: 0.0,
        });
    }
    ...
    let rationale = format!(
        ...
        p.split_ratio * 100.0,
        best.name,
        best.is_return * 100.0,
        best.is_sharpe,
        best.is_mdd * 100.0,
        (1.0 - p.split_ratio) * 100.0,
        best.oos_return * 100.0,
        best.oos_sharpe,
        best.oos_mdd * 100.0,
        diagnosis.trend,
        diagnosis.signal,
...
pub fn rank_top(
    mut recs: Vec<StockRecommendation>,
    p: &RecommendParams,
    ...
    let zr = zscores(
        &recs
            .iter()
            .map(|r| r.best_strategy.oos_return)
            .collect::<Vec<_>>(),
    );
    let zs = zscores(
        &recs
            .iter()
            .map(|r| r.best_strategy.oos_sharpe)
            .collect::<Vec<_>>(),
    );
    let zm = zscores(
        &recs
            .iter()
            .map(|r| r.best_strategy.oos_mdd)
            .collect::<Vec<_>>(),
    );
    let w = &p.weights;
    ...
    G: FnMut(&str) -> GateOutcome,
{
    ...
    for &code in pool {
        let (gate_status, profile) = match gate(code) {
            GateOutcome::Excluded(e) => {
                gate_excluded.push((code.to_string(), e.reason()));
                continue;
            }
...
mod tests {
    ...
    fn load_two(code: &str) -> anyhow::Result<Vec<StockBar>> {
        match code {
            "600519" => Ok(series(
                &(0..300).map(|i| 100.0 + i as f64 * 0.5).collect::<Vec<_>>(),
            )),
            _ => Ok(series(
                &(0..300).map(|i| 100.0 + i as f64 * 0.2).collect::<Vec<_>>(),
            )),
        }
    }
    ...
    fn dummy_profile() -> Profile {
        Profile {
            ...
            name: "茅台".into(),
            years: 10,
            roe_median: Some(30.0),
            roe_streak: 10,
            revenue_cagr: Some(20.0),
            profit_cagr: Some(20.0),
            gross_margin: Some(90.0),
            market_cap: 1.5e12,
            pe_ttm: Some(18.0),
            pe_percentile: Some(0.3),
            note: "测试画像".into(),
        }
    }
    ...
    fn evaluate_stock_ok_on_uptrend() {
        ...
        assert!(
            (r.best_strategy.score - max).abs() < 1e-9,
            "best 应为最高分"
        );
    }
    ...
    fn evaluate_stock_too_short_errors() {
        ...
        let err =
            evaluate_stock("x", "x", &series(&vals), &RecommendParams::default()).unwrap_err();
        assert!(
    ...
    fn build_report_ranks_and_skips() {
        let names: HashMap<String, String> = [("600519".to_string(), "茅台".to_string())]
            .into_iter()
            .collect();
    ...
    fn report_serializes_frontend_keys() {
        ...
        let rep = build_report(
            ...
            |_| {
                Ok(series(
                    &(0..300).map(|i| 100.0 + i as f64 * 0.4).collect::<Vec<_>>(),
                ))
            },
    ...
    fn gate_excludes_before_technical_ranking() {
        ...
        let rep = build_report(
            ...
            |code| {
                if code == "LOSSY" {
                    GateOutcome::Excluded(Exclusion::LossMaking)
                } else {
                    GateOutcome::Passed(dummy_profile())
                }
            },
        );
        assert!(
            rep.top.iter().all(|r| r.code != "LOSSY"),
            "被排除者不进技术排名"
        );
        assert!(
            rep.gate_excluded.iter().any(|(c, _)| c == "LOSSY"),
            "排除应记入 gate_excluded"
        );
        assert!(
            rep.top
                .iter()
                .any(|r| r.code == "600519" && r.profile.is_some()),
            "过闸者应带 Profile"
        );
    }
    ...
    fn non_a_share_kept_and_marked_not_applicable() {
        ...
        let rep = build_report(
            ...
            &RecommendParams::default(),
            load_two,
            |_| GateOutcome::NotApplicable("非A股".into()),
        );
    ...
    fn profile_does_not_affect_ranking_score() {
        ...
        for (a, b) in with_prof.top.iter().zip(without.top.iter()) {
            ...
            assert!(
                (a.stock_score - b.stock_score).abs() < 1e-12,
                "分数不受 Profile 影响"
            );
        }
    }
