...
pub fn diagnose(
    ...
    let boll_z = if b.std > 1e-12 {
        (price_adj - b.mid) / b.std
    } else {
        0.0
    };
    ...
    let window_return = if w[0] > 0.0 {
        w[w.len() - 1] / w[0] - 1.0
    } else {
        0.0
    };
    let trend = if window_return > p.up_threshold && ma_short > ma_long {
        "上涨"
...
mod tests {
    ...
    fn range_detected() {
        let vals: Vec<f64> = (0..80)
            .map(|i| 100.0 + if i % 2 == 0 { 0.0 } else { 0.5 })
            .collect();
        let dgn = diagnose(
            "600519".into(),
            "茅台".into(),
