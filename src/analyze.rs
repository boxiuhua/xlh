...
pub fn evaluate_triggers(
    ...
    band_window: usize,
    horizon: usize,
) -> Option<TriggerEvidence> {
    // 需要 band_window 个点建带 + 至少一个可测的前瞻窗口
    if points.len() < band_window + horizon + 2 {
        return None;
    }
    ...
    for t in band_window..navs.len().saturating_sub(horizon) {
        ...
        if prev > buy_line && cur <= buy_line {
            buy_f.push(r);
        }
        if prev < sell_line && cur >= sell_line {
            sell_f.push(r);
        }
    }
...
fn build_verdict(
    ...
    match (buy_mean, n_buy >= MIN_SIGNALS) {
        (Some(b), true) => {
            ...
            if edge <= 0.0 {
                parts.push(
                    "这条低吸线没有跑赢「随便哪天买」—— 按现有历史，它不提供择时价值。".into(),
                );
            } else if edge < 0.5 {
                parts.push("超额很小，与噪声难以区分；不宜据此加大仓位。".into());
            } else {
                parts.push(
                    "历史上有正超额，但这是单只基金的样本内统计，未经样本外检验，不保证延续。"
                        .into(),
                );
            }
        }
...
pub fn detect_regime(points: &[NavPoint], p: &RegimeParams) -> anyhow::Result<RegimeReport> {
    ...
    let eps = 0.005;
    let ma_relation = if ma_short > ma_long * (1.0 + eps) {
        "多头排列"
    } else if ma_short < ma_long * (1.0 - eps) {
        "空头排列"
    } else {
        "纠缠"
    };
    ...
    for win in w.windows(2) {
        if win[0].acc_nav > 0.0 {
            rets.push(win[1].acc_nav / win[0].acc_nav - 1.0);
        }
    }
...
pub fn build_action_plan(
    ...
    let next_hint = if z < 1.0 {
        format!(
            "距低吸线 累计{:.4}/单位{:.4} 需 {:+.1}%，距高抛线 累计{:.4}/单位{:.4} 需 {:+.1}%",
            buy,
            unit_of(buy),
            pct_to(buy),
            sell,
...
mod evidence_tests {
    ...
    fn reports_no_edge_when_the_line_has_none() {
        ...
        if e.buy_signals >= 10 {
            ...
            assert!(
                e.verdict.contains("超额") || e.verdict.contains("样本量不足"),
                "必须给出相对基准的超额: {}",
                e.verdict
            );
    ...
    fn band_is_recomputed_from_past_data_only() {
        ...
        let mut navs: Vec<f64> = (0..300)
            .map(|i| 1.0 + ((i % 10) as f64 - 4.5) * 0.002)
            .collect();
    ...
    fn refuses_to_conclude_on_thin_samples() {
        ...
        if let Some(ev) = e {
            if ev.buy_signals < 10 && ev.buy_signals > 0 {
                assert!(
                    ev.verdict.contains("样本量不足") || ev.verdict.contains("样本不足"),
                    "样本少时须明说不能判断: {}",
                    ev.verdict
                );
            }
    ...
    fn none_when_history_too_short() {
        ...
        assert!(
            evaluate_triggers(&pts(&navs), 60, 20).is_none(),
            "不足以建带 → None"
        );
    }
    ...
    fn verdict_admits_the_thresholds_are_arbitrary() {
        let navs: Vec<f64> = (0..800)
            .map(|i| 1.0 + ((i % 40) as f64 - 20.0) * 0.003)
            .collect();
        let e = evaluate_triggers(&pts(&navs), 60, 20).unwrap();
        assert!(
            e.verdict.contains("经验取值") && e.verdict.contains("未经寻优验证"),
            "须承认 60日/±1σ 是拍脑袋的: {}",
            e.verdict
        );
    }
...
mod tests {
    ...
    fn detects_range() {
        ...
        let vals: Vec<f64> = (0..130)
            .map(|i| if i % 2 == 0 { 1.00 } else { 1.01 })
            .collect();
        let r = detect_regime(&series(&vals), &RegimeParams::default()).unwrap();
    ...
    fn report_with(regime: &str) -> RegimeReport {
        RegimeReport {
            ...
            ma_long: 1.0,
            ma_relation: "纠缠".into(),
            rec_strategy: "rsi".into(),
            rec_name: "RSI超买超卖".into(),
            rationale: "".into(),
            plan: None,
        }
    }
    ...
    fn plan_deep_below_is_strong_buy() {
        ...
        let mut vals: Vec<f64> = (0..59)
            .map(|i| if i % 2 == 0 { 0.99 } else { 1.01 })
            .collect();
        vals.push(0.5);
        let plan = build_action_plan(&series(&vals), &report_with("震荡"), &PlanParams::default())
            .unwrap();
        assert_eq!(
            plan.current.signal, "强力低吸",
            "深跌应判强力低吸: z={}",
            plan.current.z
        );
        assert!(
            plan.current.action.contains("2000"),
            "应买入 2× 基准=2000: {}",
            plan.current.action
        );
    }
    ...
    fn plan_high_above_is_sell() {
        ...
        let plan = build_action_plan(&series(&vals), &report_with("震荡"), &PlanParams::default())
            .unwrap();
        assert!(
            plan.current.signal.contains("高抛"),
            "高位应判高抛: {}",
            plan.current.signal
        );
        assert!(
            plan.current.action.contains("卖"),
            "应给出卖出动作: {}",
            plan.current.action
        );
    }
    ...
    fn plan_caveat_varies_by_regime() {
        let vals: Vec<f64> = (0..60)
            .map(|i| 1.0 + 0.05 * ((i as f64) * 0.5).sin())
            .collect();
        let pts = series(&vals);
        let up = build_action_plan(&pts, &report_with("上涨趋势"), &PlanParams::default()).unwrap();
        let down =
            build_action_plan(&pts, &report_with("下跌趋势"), &PlanParams::default()).unwrap();
        let range = build_action_plan(&pts, &report_with("震荡"), &PlanParams::default()).unwrap();
        assert!(
            up.caveat.contains("顺势") || up.caveat.contains("上涨"),
            "上涨提示: {}",
            up.caveat
        );
        assert!(
            down.caveat.contains("风险") || down.caveat.contains("下跌"),
            "下跌提示: {}",
            down.caveat
        );
        assert!(
            range.caveat.contains("震荡") || range.caveat.contains("高抛低吸"),
            "震荡提示: {}",
            range.caveat
        );
    }
    ...
    fn plan_reports_unit_nav_equivalents() {
        ...
        let acc: Vec<f64> = (0..60)
            .map(|i| 2.2 + 0.02 * ((i as f64) * 0.5).sin())
            .collect();
        ...
        assert!(
            (plan.current.nav - plan.current.unit_nav - 1.5).abs() < 1e-6,
            "累计应比单位高 1.5"
        );
        // 每条触发线的等价单位净值 = 累计净值 − 1.5
        for t in &plan.tiers {
            assert!(
                (t.unit_nav - (t.nav - 1.5)).abs() < 1e-6,
                "{} 等价单位净值应为累计−1.5",
                t.label
            );
        }
    ...
    fn plan_uptrend_softens_sell_in_high_zone() {
        ...
        let up = build_action_plan(&pts, &report_with("上涨趋势"), &PlanParams::default()).unwrap();
        assert_eq!(
            range.current.signal, "高抛",
            "震荡高抛区: z={}",
            range.current.z
        );
        assert!(
            range.current.action.contains("卖"),
            "震荡应卖出: {}",
            range.current.action
        );
        assert_eq!(
            up.current.signal, "高抛",
            "上涨同样在高抛区: z={}",
            up.current.z
        );
        assert!(
            up.current.action.contains("持有"),
            "上涨趋势下高抛区应改持有: {}",
            up.current.action
        );
    }
