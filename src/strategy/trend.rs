...
impl Strategy for Trend {
    fn on_market(&mut self, ctx: &StrategyContext) -> Vec<SignalEvent> {
        let (ms, ml) = match (
            moving_average(ctx.history, self.short),
            moving_average(ctx.history, self.long),
        ) {
            (Some(s), Some(l)) => (s, l),
            _ => return Vec::new(),
        };
