...
mod tests {
    ...
    fn take_profit_triggers_all_out() {
        ...
        let ctx = StrategyContext{today:&today, history:std::slice::from_ref(&today), shares:100.0, avg_cost:1.0, cash:0.0};
    ...
    fn stop_loss_on_drawdown_from_peak() {
        ...
        let _ = s.on_market(&StrategyContext{today:&peak, history:std::slice::from_ref(&peak), shares:100.0, avg_cost:1.0, cash:0.0});
        let drop = MarketEvent{date:d(2024,1,2),nav:1.5,adj_nav:1.5}; // 市值150 vs 峰200 → -25%
        let sigs = s.on_market(&StrategyContext{today:&drop, history:std::slice::from_ref(&drop), shares:100.0, avg_cost:1.0, cash:0.0});
