...
impl Strategy for RuleLayer {
    fn on_market(&mut self, ctx: &StrategyContext) -> Vec<SignalEvent> {
        ...
        let Some(last_nav) = ctx.last_nav() else {
            return sigs;
        }; // 首日无历史 → 无从判断

        let pos_value = ctx.shares * last_nav;
        if pos_value > self.peak_value {
            self.peak_value = pos_value;
        }
...
mod tests {
    ...
    impl Strategy for Noop {
        fn on_market(&mut self, _: &StrategyContext) -> Vec<SignalEvent> {
            Vec::new()
        }
    }
    ...
    fn take_profit_triggers_all_out() {
        let mut s = RuleLayer::new(
            Box::new(Noop),
            vec![Rule::TakeProfit { target_return: 0.3 }],
        );
        ...
        let ctx = StrategyContext {
            today: today.date,
            history: std::slice::from_ref(&today),
            shares: 100.0,
            avg_cost: 1.0,
            cash: 0.0,
        };
        ...
        assert!(sigs
            .iter()
            .any(|x| x.direction == Direction::Sell && x.amount == SignalAmount::AllOut));
    }
    ...
    fn stop_loss_on_drawdown_from_peak() {
        ...
        // 先记录高点市值，再跌破20%
        let peak = MarketEvent {
            date: d(2024, 1, 1),
            nav: 2.0,
            adj_nav: 2.0,
        };
        let _ = s.on_market(&StrategyContext {
            today: peak.date,
            history: std::slice::from_ref(&peak),
            shares: 100.0,
            avg_cost: 1.0,
            cash: 0.0,
        });
        let drop = MarketEvent {
            date: d(2024, 1, 2),
            nav: 1.5,
            adj_nav: 1.5,
        }; // 市值150 vs 峰200 → -25%
        let sigs = s.on_market(&StrategyContext {
            today: drop.date,
            history: std::slice::from_ref(&drop),
            shares: 100.0,
            avg_cost: 1.0,
            cash: 0.0,
        });
