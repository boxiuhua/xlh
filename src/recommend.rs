...
fn regime_or_fallback(points: &[NavPoint]) -> RegimeReport {
    ...
    if let Ok(r) = analyze::detect_regime_with_plan(points, &rp, &pp) {
        return r;
    }
    if let Ok(r) = analyze::detect_regime(points, &rp) {
        return r;
    }
    RegimeReport {
        regime: "数据不足".into(),
        ...
        ma_long: 0.0,
        ma_relation: "未知".into(),
        rec_strategy: String::new(),
        rec_name: String::new(),
        rationale: "数据不足，暂不给出形态与择时点".into(),
        plan: None,
    }
}
...
mod tests {
    ...
    fn evaluate_fund_too_short_errors() {
        ...
        let err =
            evaluate_fund("000001", "x", &series(&vals), &RecommendParams::default()).unwrap_err();
        assert!(
    ...
    fn build_report_ranks_and_skips() {
        // 池：两只可分析（不同斜率）+ 一只加载失败
        let names: HashMap<String, String> = [("000001".to_string(), "甲".to_string())]
            .into_iter()
            .collect();
        ...
        let rep = build_report(
            ...
            |code| match code {
                "000001" => Ok(series(
                    &(0..300).map(|i| 1.0 + i as f64 * 0.005).collect::<Vec<_>>(),
                )),
                "000002" => Ok(series(
                    &(0..300).map(|i| 1.0 + i as f64 * 0.002).collect::<Vec<_>>(),
                )),
