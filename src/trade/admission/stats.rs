...
pub struct TradeUnit {
    ...
    /// 与实际了结先后可能略有出入(分批成交交错时):对笔数与胜率无影响,连亏在
    /// 同一代码的两张工单分批交错时可能差 1(判定是「超过回测连亏 × 倍数」,影响可忽略)。
        ...
        let qty = f.qty as f64;
        {
            ...
            match f.side {
                ...
                Direction::Sell if slot.1 <= 0.0 => {
                    ...
                    // 只在完全没有在册成本时补,两处已知边界都是**偏严**(回撤被高估,
                    // 宁可误暂停也不放过):部分在册(持 500 卖 1000)仍按持仓封顶;
                    // 旧持仓分多笔出清时每笔各自补票,分母只记单笔而非整个底仓,
                    // 回撤约被放大「出清笔数」倍。
                    if let Some(r) = f.realized_pnl {
