use crate::data::{cache, eastmoney, NavPoint};
use serde::Serialize;
use std::path::Path;
    ...
    let fresh = match eastmoney::fetch(code) {
        ...
        Err(e) => {
            return SyncOutcome {
                ...
                added: 0,
                total: cached.len(),
                latest: cached.last().map(|p| p.date.to_string()),
                error: Some(format!("抓取失败: {e}")),
            }
        }
    };
...
mod tests {
    ...
    fn np(dt: NaiveDate, v: f64) -> NavPoint {
        NavPoint {
            date: dt,
            nav: v,
            acc_nav: v,
    ...
    fn empty_cache_takes_all_fresh() {
        let (merged, added) =
            merge_incremental(&[], vec![np(d(2024, 1, 1), 1.0), np(d(2024, 2, 1), 1.1)]);
        assert_eq!(added, 2);
        assert_eq!(merged.len(), 2);
    }
