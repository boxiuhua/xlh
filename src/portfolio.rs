...
use crate::event::{
    Direction, FillEvent, MarketEvent, OrderEvent, OrderQty, SignalAmount, SignalEvent,
};
...
impl Portfolio {
    ...
    pub fn on_signal(
        ...
        match (sig.direction, sig.amount) {
            (Direction::Buy, SignalAmount::Cash(c)) if c > 0.0 => Some(OrderEvent {
                date: sig.date,
                direction: Direction::Buy,
                qty: OrderQty::Cash(c),
            }),
            (Direction::Sell, SignalAmount::AllOut) if pos.shares > 1e-9 => Some(OrderEvent {
                date: sig.date,
            ...
            (Direction::Sell, SignalAmount::Ratio(r)) if pos.shares > 1e-9 && r > 0.0 => {
                ...
                Some(OrderEvent {
                    date: sig.date,
                    direction: Direction::Sell,
                    qty: OrderQty::Shares(s),
                })
            }
            (Direction::Sell, SignalAmount::Cash(c))
                if pos.shares > 1e-9 && c > 0.0 && today.adj_nav > 0.0 =>
            {
...
mod tests {
    use super::*;
    use crate::broker::Position;
    use crate::event::{Direction, FillEvent, MarketEvent, OrderQty, SignalAmount, SignalEvent};
    use chrono::NaiveDate;
        ...
        let buy = p
            .on_signal(
                &SignalEvent {
                    date: d(2024, 1, 1),
                    direction: Direction::Buy,
                    amount: SignalAmount::Cash(500.0),
                },
                &pos,
                &today,
            )
