...
use super::{build_strategy_from_fields, AppError, StrategyFields};
...
fn stock_cache() -> &'static Path {
    Path::new(".cache/stock")
}

/// 预设股票池（A股/港股/美股混合），可增删。
const STOCK_POOL: &[&str] = &[
    "600519", "000858", "601318", "300750", "000333", "600036", "002415", "600276", "00700",
    "09988", "AAPL", "MSFT",
];
...
) -> std::result::Result<Json<StockDiagnosis>, AppError> {
...
pub async fn run_handler(
    Query(q): Query<StockRunQuery>,
) -> std::result::Result<Json<StockRunOutcome>, AppError> {
...
) -> std::result::Result<Json<StockRecommendReport>, AppError> {
...
fn recommend_blocking(q: RecommendQuery) -> StockRecommendReport {
    ...
    let gate = |code: &str| -> recommend::GateOutcome {
        ...
        let reports = match fundamentals::load_or_fetch(
            ...
            Ok(r) => r,
            Err(_) => {
                return recommend::GateOutcome::NotApplicable("基本面数据获取失败,未经闸门".into())
            }
        };
        // 港股无估值历史；A 股正常应有，取不到则空序列（PE 分位因子自动降级）。
        let vals =
            valuation::load_or_fetch(code, valuation_cache(), trade_date).unwrap_or_default();
...
fn fundamentals_cache() -> &'static Path {
    Path::new(".cache/fundamentals")
}
fn valuation_cache() -> &'static Path {
    Path::new(".cache/valuation")
}
fn universe_cache() -> &'static Path {
    Path::new(".cache")
}
...
) -> std::result::Result<Json<ScreenReport>, AppError> {
...
fn screen_blocking(q: ScreenQuery) -> Result<ScreenReport> {
    ...
    Ok(screen::build_report(
        ...
        |l| {
            ...
            let vals =
                valuation::load_or_fetch(&l.code, valuation_cache(), date).unwrap_or_default();
            Ok((reports, vals))
        },
    ))
}
...
pub async fn attribution_handler(
    Query(q): Query<AttributionQuery>,
) -> std::result::Result<Json<Attribution>, AppError> {
...
fn attribution_blocking(q: AttributionQuery) -> Result<Attribution> {
    ...
    let bars = cache::load_or_fetch(
        &q.code,
        stock_cache(),
        end - chrono::Duration::days(9000),
        end,
    )
    .map_err(|e| anyhow!("加载行情失败: {e}"))?;
    ...
    attribution::attribute(&bars, &vals, start, end).map_err(|e| anyhow!("无法归因: {e}"))
}
...
mod tests {
    ...
    async fn search_route_returns_json_array() {
        let resp = crate::web::core_router()
            .oneshot(
                Request::builder()
                    .uri("/api/stock/search?q=xyzzy_no_such")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
    ...
    async fn realtime_movers_bad_date_is_400() {
        let resp = crate::web::core_router()
            .oneshot(
                Request::builder()
                    .uri("/api/stock/realtime/movers?day=garbage")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
    ...
    async fn diagnose_bad_code_is_400() {
        let resp = crate::web::core_router()
            .oneshot(
                Request::builder()
                    .uri("/api/stock/diagnose?code=bad!!code")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
