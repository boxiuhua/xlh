...
fn validate_common(fund: &str, start: NaiveDate, end: NaiveDate, buy_rate: f64) -> Result<()> {
    if start >= end {
        return Err(anyhow!(
            "回测区间错误: start ({start}) 必须早于 end ({end})"
        ));
    }
...
fn strategy_params_table(s: &StrategyFields) -> Result<toml::Table> {
    ...
    let need = |o: bool, name: &str| -> Result<()> {
        if o {
            ...
            Err(anyhow!("策略 {} 缺少必填参数: {}", s.strategy, name))
        }
    };
    match s.strategy.as_str() {
        ...
        "trend" => {
            ...
            t.insert(
                "short_window".into(),
                (s.short_window.unwrap() as i64).into(),
            );
...
pub struct OptimizeRequest {
    ...
    pub strategy: String,
    pub metric: String,
    #[serde(default = "default_top_n_web")]
    pub top_n: usize,
...
pub fn build_optimize_cfg(req: &OptimizeRequest) -> Result<crate::config::OptimizeCfg> {
    ...
    for &name in keys {
        ...
        let vals = parse_csv_values(csv).map_err(|e| anyhow!("参数 {name} 取值非法: {e}"))?;
...
pub fn build_run_from_query(q: &RunQuery) -> Result<RunSpec> {
    ...
    let sf = StrategyFields {
        ...
        ma_window: q.ma_window,
        k: q.k,
        short_window: q.short_window,
        long_window: q.long_window,
        amount: q.amount,
        rsi_window: q.rsi_window,
        oversold: q.oversold,
    ...
    let fee = FeeModel {
        buy_rate: q.buy_rate,
        sell_tiers: standard_sell_tiers(),
    };
    Ok(RunSpec {
    ...
    S: Clone + Send + Sync + 'static,
{
    Router::new()
        ...
        .route(
            "/api/stock/realtime/movers",
            get(stock::realtime_movers_handler),
        )
}
...
fn push_user_routes() -> Router<auth::AuthState> {
    Router::new()
        .route(
            "/api/push/config",
            get(push_config_get).post(push_config_save),
        )
...
/// 仅供反向代理、容器与监控系统探测；不返回用户数据或业务配置。
async fn healthz(State(st): State<AuthState>) -> Response {
    let now = chrono::Local::now();
    let (database_ok, push_alive, push_last_beat) = {
        let Ok(conn) = st.db.lock() else {
            return (
                StatusCode::SERVICE_UNAVAILABLE,
                axum::Json(serde_json::json!({
                    "status": "unhealthy", "database": "lock_poisoned"
                })),
            )
                .into_response();
        };
        let database_ok = conn.query_row("SELECT 1", [], |_| Ok(())).is_ok();
        let push_alive = crate::push::store::daemon_alive(&conn, now);
        let push_last_beat = crate::push::store::last_beat(&conn)
            .ok()
            .flatten()
            .map(|t| t.to_rfc3339());
        (database_ok, push_alive, push_last_beat)
    };
    let realtime_path = crate::stock::realtime::config::get().db_path.clone();
    let realtime_ok = tokio::task::spawn_blocking(move || {
    ...
    (
        ...
        axum::Json(serde_json::json!({
            ...
            "realtime_database": if realtime_ok { "ok" } else { "unavailable" },
            "push_daemon_alive": push_alive,
            "push_last_beat": push_last_beat,
        })),
    )
        .into_response()
}
...
fn compare_blocking(req: CompareRequest) -> Result<String> {
    ...
    for run in &req.runs {
        let fund = run
            .fund_code
            .clone()
            .unwrap_or_else(|| req.fund_code.clone());
        validate_fund_code(&fund)?;
    ...
    let meta = crate::report::compare::CompareMeta {
        start: req.start,
        end: req.end,
    };
    Ok(crate::report::compare::render_compare_html(
        &meta, &outcomes,
    ))
}
...
fn optimize_blocking(req: OptimizeRequest) -> Result<String> {
    ...
    let report =
        crate::optimize::run_optimize(&cfg, &req.fund_code, &points, fee, req.initial_cash)?;
    let meta = crate::report::optimize::OptMeta {
        start: req.start,
        end: req.end,
        fund_code: req.fund_code.clone(),
    };
    Ok(crate::report::optimize::render_optimize_html(
        &meta, &report,
    ))
}
...
fn regime_blocking(q: RegimeQuery) -> Result<crate::analyze::RegimeReport> {
    ...
    let points =
        crate::data::cache::load_or_fetch(&q.fund_code, std::path::Path::new(".cache"), start, end)
            .map_err(|e| anyhow!("加载净值失败: {e}"))?;
    let params = crate::analyze::RegimeParams {
        window,
        ..Default::default()
    };
...
const PRESET_POOL: &[&str] = &[
    "161725", "050002", "000834", "001427", "003095", "008888", "110011", "005827", "110022",
    "161005", "163406", "260108", "000961", "001593", "519674", "320007", "002001", "001714",
...
fn recommend_blocking(q: RecommendQuery) -> Result<crate::recommend::RecommendReport> {
    ...
    let names: std::collections::HashMap<String, String> =
        funds_payload(std::path::Path::new(".cache"))
            .into_iter()
            .map(|f| (f.code, f.name))
            .collect();
    let end = chrono::Local::now().date_naive();
    let start = end - chrono::Duration::days(8 * 365);
    let report =
        crate::recommend::build_report(PRESET_POOL, &names, &end.to_string(), &params, |code| {
            crate::data::cache::load_or_fetch(code, std::path::Path::new(".cache"), start, end)
        });
    Ok(report)
}
...
fn holdings_history_routes() -> Router<auth::AuthState> {
    Router::new()
        ...
        .route(
            "/api/holdings/history/:id",
            get(holdings_history_detail_handler),
        )
}
...
async fn push_status(
    ...
    let last_beat = crate::push::store::last_beat(&conn)
        .ok()
        .flatten()
        .map(|t| t.to_rfc3339());
    ...
    if !alive {
        blockers.push(
            "推送守护进程未运行 —— 配置保存了也不会推送。\
             启动：docker compose -f docker-compose.prod.yml --profile push up -d"
                .into(),
        );
    }
    if let Some(c) = &cfg {
...
async fn push_preview(
    ...
    let (md, has_new) = tokio::task::spawn_blocking(move || crate::push::build_message(&cfg))
        ...
        .map_err(|e| anyhow!("任务执行失败: {e}"))??;
    Ok(axum::Json(
        serde_json::json!({"markdown": md, "has_new": has_new}),
    ))
}
...
mod tests {
    ...
    fn build_strategy_from_fields_missing_param() {
        ...
        assert!(
            err.to_string().contains("ma_window"),
            "应提示缺 ma_window: {err}"
        );
    }
    ...
    fn rejects_start_after_end() {
        ...
        assert!(
            err.to_string().contains("start") || err.to_string().contains("区间"),
            "应提示区间错误: {err}"
        );
    }
    ...
    fn rejects_unknown_strategy() {
        ...
        let err = build_run_from_query(&q).unwrap_err();
        assert!(
            err.to_string().contains("bogus") || err.to_string().contains("策略"),
            "应提示未知策略: {err}"
        );
    }
    ...
    fn build_optimize_cfg_missing_param() {
        ...
        assert!(
            err.to_string().contains("base_amount"),
            "应提示缺 base_amount: {err}"
        );
    }
    ...
    fn seed_licensed(state: &crate::web::auth::AuthState, name: &str, token: &str) -> i64 {
        ...
        crate::web::auth::store::create_session(&c, token, uid, now + chrono::Duration::days(1))
            .unwrap();
        uid
    }
    async fn push_post(
        state: crate::web::auth::AuthState,
        uri: &str,
        token: &str,
    ...
    async fn compare_empty_runs_is_400() {
        ...
        assert_eq!(
            post_json("/api/compare", body).await,
    ...
    async fn optimize_missing_grid_param_is_400() {
        ...
        assert_eq!(
            post_json("/api/optimize", body).await,
            axum::http::StatusCode::BAD_REQUEST
        );
    }
    ...
    fn index_has_adaptive_option() {
        ...
        assert!(
            body.contains("value=\"adaptive\""),
            "index should have adaptive option"
        );
    ...
    async fn regime_route_bad_code_is_400() {
        ...
        let resp = super::core_router()
            .oneshot(
                Request::builder()
                    .uri("/api/regime?fund_code=bad!!code")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
    ...
    async fn push_config_get_returns_default_for_new_user() {
        ...
        let resp = super::router(state)
            .oneshot(
                Request::builder()
                    .uri("/api/push/config")
                    .header("cookie", "xlh_session=ptok")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
    ...
    async fn push_config_requires_auth() {
        ...
        let state = push_state();
        {
            ...
            let now = chrono::Local::now().date_naive();
            crate::web::auth::store::create_session(
                &c,
                "ntok",
                uid,
        ...
        let r2 = super::router(state)
            .oneshot(
                Request::builder()
                    .uri("/api/push/config")
                    .header("cookie", "xlh_session=ntok")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
    ...
    async fn push_config_save_empty_webhook_is_400() {
        ...
        assert_eq!(
            push_post(state, "/api/push/config", "ptok", body).await,
            axum::http::StatusCode::BAD_REQUEST
        );
    }
    ...
    async fn push_config_save_then_get_and_cache_dir_hardened() {
        ...
        assert_eq!(
            push_post(state.clone(), "/api/push/config", "ptok", body).await,
        ...
        let v: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
        assert_eq!(
            v["channel"]["webhook"], "https://open.feishu.cn/x",
            "读回一致"
        );
        assert_eq!(
            v["channel"]["cache_dir"], ".cache",
            "cache_dir 被服务端覆盖"
        );
    }
    ...
    async fn holdings_empty_returns_valid_report() {
        ...
        assert!(
            v["advices"].as_array().unwrap().is_empty(),
            "空持仓 advices 为空"
        );
    ...
    async fn holdings_save_requires_login() {
        ...
        let resp = app
            .oneshot(
                Request::builder()
                    .method("POST")
                    .uri("/api/holdings/save")
                    .header("content-type", "application/json")
                    .body(Body::from("{\"holdings\":[]}"))
                    .unwrap(),
            )
            .await
            .unwrap();
    ...
    async fn holdings_save_then_history_for_active_user() {
        ...
        let token = "tok-hist".to_string();
        {
            ...
            crate::web::auth::store::set_expiry(
                &c,
                uid,
        ...
        let bytes = axum::body::to_bytes(list.into_body(), 1_000_000)
            .await
            .unwrap();
