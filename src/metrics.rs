...
pub fn total_return(final_equity: f64, total_contributed: f64) -> f64 {
    if total_contributed > 0.0 {
        final_equity / total_contributed - 1.0
    } else {
        0.0
    }
}
...
mod tests {
    ...
    fn summarize_fields() {
        ...
        assert!(
            (s.total_return - 1.0).abs() < 1e-9,
            "total_return should be 1.0 (100%)"
        );
        assert!(
            s.max_drawdown >= 0.0 && s.max_drawdown < 0.5,
            "max_drawdown in valid range"
        );
