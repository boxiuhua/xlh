...
mod tests {
    ...
    fn run_one_dca_flat_then_up() {
        ...
        assert!(
            (outcome.summary.final_equity - 4000.0).abs() < 1e-6,
            "expected final_equity≈4000, got {}",
            outcome.summary.final_equity
        );
        assert!(
            (outcome.summary.total_contributed - 2000.0).abs() < 1e-6,
            "expected total_contributed≈2000, got {}",
            outcome.summary.total_contributed
        );
