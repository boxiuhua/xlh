...
impl<D: DataHandler, S: Strategy> Engine<D, S> {
    ...
    pub fn run(&mut self) -> &Portfolio {
        ...
        while let Some(today) = self.data.next_bar() {
            if !seeded {
                self.portfolio.seed(today.date);
                seeded = true;
            }
            ...
            self.portfolio
                .record_equity(today.date, self.broker.total_shares(), today.adj_nav);
...
mod tests {
    ...
    fn take_profit_exits_through_engine() {
        ...
        assert!(
            last.equity > 0.0,
            "portfolio must have positive equity after take-profit sell; got {}",
            last.equity
        );
        // Cash must be positive (proceeds from the sale).
        assert!(
            pf.cash > 0.0,
            "cash must be positive after take-profit sell; got {}",
            pf.cash
        );
        // Total contributed must equal the single DCA buy (1000).
        assert!(
            (pf.total_contributed - 1000.0).abs() < 1e-6,
            "expected 1000 contributed, got {}",
    ...
    fn captures_daily_and_trades() {
        ...
        assert!(
            (last_equity - 4000.0).abs() < 1e-6,
            "expected last equity≈4000, got {}",
            last_equity
        );
