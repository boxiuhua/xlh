...
> 核心原则：**复用现有 engine/broker/portfolio/metrics/recommend，股票以适配器接入**，不重写、不改动能跑的基金代码。
> 本文档只覆盖**子项目 1：股票数据层**。
...
新增 `src/data/stock/` 子目录；`src/data/mod.rs`（或 `src/lib.rs`）仅加一行 `pub mod stock;`。
...
- `src/data/stock/` 六个模块落地，`cargo build` 通过。
- 上述离线单测全部通过。
