...
**持仓录入**:`holdings.rs` 的持仓是请求期输入、从未落库,无可导入数据;用户首次使用时通过交易页「持仓校准」录入实盘持仓(每次校准留痕,见 `trade_position_adjusts`)。
...
| manual | 个股诊断 / AI 分析结果「生成工单」 | 当日 15:00(交易时段内生成) | AI 内容自动填入理由;同一闸门;仅交易时段(交易日 9:30–11:30、13:00–15:00)内生成,需今日新鲜行情(≤60 秒,与确认同一口径),否则 400 `no_quote`;`request_id` 幂等 |
